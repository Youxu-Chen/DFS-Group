...
void Client::corefs_print_xattrs(Inode *in){
  ...
  //   s_tmp = (char *)tmp;
  //   s_tmp = s_tmp + "\0";
  //   ldout(cct, 1) << "corefs: get xattrs <" << s_name << ", " << s_tmp << "> from ino " << in->ino << dendl;
  // }
}
...
void Client::corefs_set_xattrs(Inode *in, const char *buf, uint64_t size){
  ...
  in->xattrs.insert(pair<string, bufferptr>(key, NULL));
  in->xattr_version++;
  ...
  //   ldout(cct, 1) << __func__ << " start setxattr" << dendl;
  //   in->xattr_version++;
  //   corefs_print_xattrs(in);
  //   // _do_setxattr(in,s.c_str(),buf,size,CEPH_XATTR_CREATE,in->uid,in->gid);
  // }

  ldout(cct, 1) << __func__ << " set xattrs <" << key << ", ""> into ino " << in->ino << " done." << dendl;
}
