    ...
    if(reply->head.is_target >= 2){
      ...
      while(p != reply->get_trace_bl().end()){
        ...
        while(i < num_dn){
          ...
          // ldout(cct, 2) << __func__ << " corefs.prefetched dn's inode=" << *diri_phd << dendl;
        ...
        // ldout(cct, 2) << __func__ << " corefs.prefetched inode=" << *in_phd << dendl;
        in_singlephd.push_back(in_phd);

        // corefs writing to metadata cache
        i = 0;
        vector<Inode*>::iterator v_iter = in_singlephd.begin();
        for(; v_iter != in_singlephd.end(); v_iter++)
          ldout(cct, 2) << " corefs_insert_trace " << *(*v_iter) << dendl;
        ...
        while(i < in_singlephd.size()-1){
          ...
          insert_dentry_inode(dir_phd, vdname_phd[i_dir], &vdlease_phd[i_dir], in_singlephd[i_in], request->sent_stamp, session,
...
success:
 ...
 // corefs
    // ldout(cct, 1) << "writing data:" << buf << dendl;
    char tmp[1024];
    int len,r_sscanf;
    len = 0;
    ...
    if(r_sscanf){
      ...
      len = strlen(tmp);
      corefs_set_xattrs(in,tmp,len);
    }
    
    while(sscanf(buf+len+5, "src=%s", tmp) > 0){
      ldout(cct, 1) << tmp << dendl;
      size = strlen(tmp);
      corefs_set_xattrs(in,tmp,size);
      len += size;
    }
